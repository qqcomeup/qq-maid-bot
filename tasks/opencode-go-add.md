...
本任务需要新增并接线普通聊天专用候选链配置。下面是目标实现完成后的配置示例；
当前仓库尚未读取该变量，实现前不要把它当作可直接生效的运行配置：
...
* 超时、限流、5xx、连接失败等可恢复错误继续尝试后续 Provider；
* 401、403 等鉴权错误继续作为配置错误暴露，不跨模型降级；
...
* 429 时不进行持续重试；
* 401、403 鉴权失败按配置错误处理，启动期或调用期明确暴露，不标记为可 fallback 的临时不可用；
* 5xx 可以按现有策略重试一次；
* 空输出视为调用失败；
* SSE 中断后按现有策略处理；
