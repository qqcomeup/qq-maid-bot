...
const HELP_MODULES: &[HelpModule] = &[
    ...
    HelpModule {
        ...
        commands: &[
            ...
            "- `/todo add G34 杭州东 北京南 明天`：识别火车行程，查询 12306 时刻后创建待办",
            "- `/todo done [编号...]`：查看已完成待办，或完成最近列表中的编号",
            "- `/todo undo [编号...]`：查看已完成待办，或恢复最近已完成列表中的编号",
            "- `/todo edit 目标 新内容`：修改列表编号或关键词匹配的待办",
            "- `/todo delete 目标`：删除列表编号或关键词匹配的待办；`delete done` 清理已完成待办",
        ],
        notes: &[
            "- 新增、修改和删除会先展示草稿或目标，确认后再写入。",
            "- `done` 的编号来自最近一次 `/todo` 列表；`undo` 的编号来自最近一次已完成列表。",
            "- 火车行程会自动查询 12306 校验车次、站点和时间；座位号和站台为可选信息。",
        ],
    },
    HelpModule {
        key: "rss",
